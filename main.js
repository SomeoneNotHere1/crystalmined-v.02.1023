...
let placeOptions=[
	...
	{//3
		...
		price:50,
		x:10,
		y:(50+75)+(75+75)+75,//the power of being lazy and putting stuff off for the future will ensure this is never changed
		draw(){
       	var c=document.getElementById("myCanvas");
        	var ctx=c.getContext("2d");
        	var imagee = new Image();
      ...
      var imagee = new Image();
		...
		if(gameInfo.placing && !explainDEA){
			...
			if(is_mouse_in_area()){
				if(gameInfo.placing == 'upgrade.png'){
					...
					if(targ.texture=='generator.png'){
						targ.texture='generatorAuto.png'	
					}
					else if(targ.texture=='generatorAuto.png'){
		...
		if(targ.texture=='unloader.png'){
			if(floor[i-1]!=null && floor[i-1].texture=='barrel.png'){
				if(floor[i-1].itemsStored>=1){
					if(floor[i+1].cooldown>=40){
						floor[i+1].cooldown=0
						floor[i-1].itemsStored--
						floor[i+1].overlayTexture='item1.png'
					}
					else{
						floor[i+1].cooldown++
